...
document.addEventListener('DOMContentLoaded', () => {
    const iconPaths = {
        ...
        'fa-trophy': '<path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.45 1-1 1H4v2h16v-2h-5c-.55 0-1-.45-1-1v-2.34"/><path d="M12 2a5 5 0 0 1 5 5v3a5 5 0 0 1-10 0V7a5 5 0 0 1 5-5z"/>',
    ...
    if (cursorDot && cursorOutline && window.innerWidth > 768) {
        ...
        const hoverables = document.querySelectorAll('a, button, .btn-glow, .bot-question-btn, .copy-badge, .social-btn, .project-card, .edu-card, .honor-card, .cert-card');
    ...
    if (achievementSection) {
        const achievementObserver = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.add('crackers-active');
    ...
    const boomTargets = document.querySelectorAll('.boom-target');
    boomTargets.forEach(target => {
        const triggerBoom = () => {
        ...
        target.addEventListener('click', triggerBoom);
        target.addEventListener('touchstart', triggerBoom, { passive: true });
        target.addEventListener('animationend', () => target.classList.remove('boom'));
    });
